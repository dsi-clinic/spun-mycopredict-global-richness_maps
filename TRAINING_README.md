...
1. **Training data:**
   - `data/20260123_arbuscular_mycorrhizal_only_alphaearth_ball.csv` - AM fungal richness training data (AlphaEarth variables only)
   - `data/20260123_ectomycorrhizal_only_alphaearth_ball.csv` - EcM fungal richness training data (AlphaEarth variables only)
...
1. **Remove the column from your training data:**
   ```bash
   # For AM data
   cut -d',' -f1-$(head -1 data/20260123_arbuscular_mycorrhizal_only_alphaearth_ball.csv | \
     tr ',' '\n' | grep -n "knndmw_CV_folds" | cut -d':' -f1 | \
     awk '{print $1-1}') data/20260123_arbuscular_mycorrhizal_only_alphaearth_ball.csv \
     > temp.csv && mv temp.csv data/20260123_arbuscular_mycorrhizal_only_alphaearth_ball.csv
   ```
...
2. **Or use a Python script:**
   ...
   df.to_csv('data/20260123_arbuscular_mycorrhizal_only_alphaearth_ball.csv', index=False)
   ```
