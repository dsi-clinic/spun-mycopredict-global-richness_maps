...
1. **Training data:**
   - `data/20260123_arbuscular_mycorrhizal_only_alphaearth_2balls_pca.csv` - AM fungal richness training data (AlphaEarth variables only)
   - `data/20260123_ectomycorrhizal_only_alphaearth_2balls_pca.csv` - EcM fungal richness training data (AlphaEarth variables only)
...
1. **Remove the column from your training data:**
   ```bash
   # For AM data
   cut -d',' -f1-$(head -1 data/20260123_arbuscular_mycorrhizal_only_alphaearth_2balls_pca.csv | \
     tr ',' '\n' | grep -n "knndmw_CV_folds" | cut -d':' -f1 | \
     awk '{print $1-1}') data/20260123_arbuscular_mycorrhizal_only_alphaearth_2balls_pca.csv \
     > temp.csv && mv temp.csv data/20260123_arbuscular_mycorrhizal_only_alphaearth_2balls_pca.csv
   ```

2. **Or use a Python script:**
   ```python
   import pandas as pd
   df = pd.read_csv('data/20260123_arbuscular_mycorrhizal_only_alphaearth_2balls_pca.csv')
   df = df.drop(columns=['knndmw_CV_folds'], errors='ignore')
   df.to_csv('data/20260123_arbuscular_mycorrhizal_only_alphaearth_2balls_pca.csv', index=False)
   ```
