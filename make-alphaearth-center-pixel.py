...
y, x = np.ogrid[:25, :25]
big_gaussian = np.exp(-((x - 12) ** 2 + (y - 12) ** 2) / (2.0 * 3**2))
big_gaussian /= big_gaussian.sum()
little_gaussian = np.exp(-((x - 12) ** 2 + (y - 12) ** 2) / (2.0 * 1**2))
...
X = am_alpha23_patches.reshape(-1, 64)
X_centered = X - np.mean(X, axis=0)
_, _, Vt = np.linalg.svd(X_centered, full_matrices=False)
X_pca_32 = (X_centered @ Vt[:32].T).reshape((len(am_alpha23_patches), 25, 25, 32))
...
am_alpha23_patches = np.concatenate([X_big, X_little], axis=1)

am_alpha23 = pd.DataFrame(
    {"sample_id": am_alpha23_ids}
    | {x: am_alpha23_patches[:, i] for i, x in enumerate(am_alpha23_band_names)},
)
...
X = em_alpha23_patches.reshape(-1, 64)
X_centered = X - np.mean(X, axis=0)
_, _, Vt = np.linalg.svd(X_centered, full_matrices=False)
X_pca_32 = (X_centered @ Vt[:32].T).reshape((len(em_alpha23_patches), 25, 25, 32))

X_big = (X_pca_32 * big_gaussian[np.newaxis, :, :, np.newaxis]).sum(axis=(1, 2))
X_little = (X_pca_32 * little_gaussian[np.newaxis, :, :, np.newaxis]).sum(axis=(1, 2))

em_alpha23_patches = np.concatenate([X_big, X_little], axis=1)

em_alpha23 = pd.DataFrame(
    {"sample_id": em_alpha23_ids}
    | {x: em_alpha23_patches[:, i] for i, x in enumerate(em_alpha23_band_names)},
)
...
ecm_alpha = ecm_alpha[
    ["sample_id"] + list(em_alpha23_band_names) + list(ecm.columns[25:])
]

am_alpha.to_csv(
    "data/20260123_arbuscular_mycorrhizal_only_alphaearth_2balls_pca.csv", index=False
)
ecm_alpha.to_csv(
    "data/20260123_ectomycorrhizal_only_alphaearth_2balls_pca.csv", index=False
)
