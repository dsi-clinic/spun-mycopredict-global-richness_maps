...
y, x = np.ogrid[:25, :25]
gaussian = np.exp(-((x-12)**2 + (y-12)**2) / (2.0*3**2))
...
am_alpha23_patches = (
    am_alpha23_patches * gaussian[np.newaxis, :, :, np.newaxis]
).sum(axis=(1,2))
...
em_alpha23_patches = (
    em_alpha23_patches * gaussian[np.newaxis, :, :, np.newaxis]
).sum(axis=(1,2))
...
am_alpha.to_csv(
    "data/20260123_arbuscular_mycorrhizal_only_alphaearth_ball.csv", index=False
)
